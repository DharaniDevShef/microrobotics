...
Designing a robot's body and its control policy together is a more fundamental problem than training a policy to make an already-fixed physical platform walk: it requires discovering the machine's geometric shape, joint distribution and dynamic behaviour at the same time. A morphology optimised purely for forward locomotion speed often exhibits poor physical stability. Similarly, a modular body that folds elegantly may fail to transmit magnetic torque effectively, while a configuration that maximises light-sensor sensitivity might sacrifice movement velocity. Physical morphology is therefore not a passive container for intelligence; it is an active part of the computation.

Traditional robotic engineering decouples these design choices into a sequential pipeline: an engineer first specifies the geometric topology, a mechanical modeller tunes joint kinematics, and a control engineer subsequently searches for a control policy that makes the completed platform behave correctly. This manual approach works for well-structured environments, but it restricts exploration to human intuition, often ruling out non-intuitive, high-performing structures before they are ever evaluated in simulation. Automated robot co-design reverses this pipeline: candidate morphologies are generated algorithmically, instantiated in a physics engine, evaluated against task-specific fitness metrics, and returned to the search loop. As illustrated in Figure~\ref{fig:automated-design-hook}, this approach combines evolutionary algorithms for macro-level structural search with reinforcement learning to evaluate or guide architectural edits.

This pattern recurs across several areas of modern robotics. Large-scale GPU environments such as NVIDIA Isaac Sim (Figure~\ref{fig:isaac-sim}) show how evaluating thousands of parallel physics instances can substantially accelerate policy learning. In physical hardware, platforms like Boston Dynamics' Atlas (Figure~\ref{fig:boston-dynamics}) illustrate how structural dynamics and high-bandwidth control must operate in tight coupling for agile, dynamic movement. Benchmarks such as Evolution Gym (Figure~\ref{fig:evolution-gym}) formalise morphology-control co-design as a research problem, separating structural generation from behavioural evaluation \citep{bhatia2021evolution,li2023modular,yang2024machine}.

The problem is harder still for modular, self-assembling systems. Unlike fixed quadruped or humanoid frames, modular units can combine into an exponential number of topological graphs. For planar systems like Roblets, each graph topology directly determines how 2D flat sheets transform into 3D folded modules (Figure~\ref{fig:self-folding}), how folded contacts interact with ground friction, and how the physical body responds to external magnetic fields or light signals. Local mechanical interlocks and light responses could also, in principle, extend towards coordination between multiple units, though this project evaluates only the single-robot reaction primitive, not swarm coordination itself (Section~\ref{sec:swarm-pheromone}).
...
Roblets are small, modular units with magnetic bonding sites and foldable sections. The units can be assembled from a two-dimensional arrangement and then folded into a three-dimensional robot. Locomotion is produced by an externally applied oscillating magnetic field rather than by motors placed in every module. This combination makes Roblets a useful test case for automated design because morphology affects assembly, folding, magnetic response and motion at the same time \citep{han2023roblets}. Figure~\ref{fig:roblet-assembly-folding-actuation} shows this assembly, folding and actuation sequence on the physical platform.
...
Simulation Runs \& Analysis & Headless parallel simulations, magnetic sweep optimisation, and convergence plotting. \\
...
\textbf{Simulation Framework Delays:}
Building the physics simulation environment and the dynamic assembly generator took nearly four weeks (early July to late July) rather than the two weeks planned. Debugging low-level physical behaviour required many rounds of code changes. The problems included hinge joint flapping, module placement in the generated XML, connector inertial mass for embedded magnets, torque actuation under an oscillating magnetic field, and fluid/glass interaction artefacts. Because both the multi-objective loop and the RL policy depend on a stable, bug-free physics evaluator, integration of the downstream framework was deliberately delayed until the simulation and its headless parallel execution had been validated, in mid-August.

\textbf{Pivot from Surrogate Fitness to Pheromone Metrics (AO-1 Adjustment):}
The original plan specified training a machine-learning surrogate model to predict locomotion fitness and speed up the evolutionary search. Once headless parallel MuJoCo simulation reached a workable throughput in mid-August (around 1.2 minutes per generation), a surrogate model was judged unnecessary, and the advanced objective was changed instead to implementing the light-response and virtual-pheromone attraction/repulsion metrics described in Section~\ref{sec:swarm-pheromone}. This kept the framework's physics-in-the-loop evaluation rather than introducing an approximation the project no longer needed.

\textbf{Delayed RL Integration:}
Evolutionary pipeline integration was originally scheduled for late July, but establishing stable multi-objective constraints (collision checks, mirror symmetry, shape entropy) took until mid-August. As a result, the PPO actor-critic for RL-guided graph edits was not implemented until late August. Initial diagnostic runs showed policy collapse, caused by overly severe penalties on invalid grammar actions and by elitism-handling issues in NSGA-III. Contingency time set aside for late August was used to redesign the penalty structure and the operator-selection routine, which stabilised the actor-critic policy in time for the results reported here.
...
Designing autonomous modular robots requires addressing both physical mechanics and algorithmic optimisation. Historically, creating functional robot morphologies depended heavily on human engineering intuition, a process that inherently restricts exploration to familiar configurations. To overcome this, the automated design of self-assembling and self-folding modular robots demands a synthesis of physical hardware platforms, evolutionary multi-objective optimisation, graph-structured representations, and reinforcement learning.

This literature review evaluates the foundational frameworks and recent advances across seven domain areas. Section 2.1 surveys the landscape of modular and self-reconfigurable robots, positioning the unique physical traits of Roblets relative to existing hardware systems. Section 2.2 examines automated robot design as a broader design-automation problem, including generative and grammar-based structural representations. Section 2.3 reviews how AI and machine learning techniques are used to guide that design search. Section 2.4 turns to evolutionary robotics and morphology-control co-design, focusing on structural representation and search-space management. Section 2.5 critically evaluates reinforcement learning techniques applied to structural search versus control optimisation. Section 2.6 analyzes multi-objective evolutionary algorithms, focusing on high-dimensional Pareto optimisation. Section 2.7 reviews graph-structured representations and graph neural network policies for variable-morphology systems. Finally, Section 2.8 synthesizes these findings to define the precise research gap addressed in this work.
...
Evolutionary Robotics (ER) applies the principles of natural selection to simultaneously synthesize a robot's physical morphology (body) and controller (brain). A broad survey of modular design automation (Li et al., 2023) categorises the field into three core challenges: representation (genotype-to-phenotype mapping), search space exploration, and controller-morphology co-optimisation \citep{li2023modular}.

A central challenge in evolving modular structures is managing a large, constrained design space. Work on evolutionary robot design also considers interactions among design variables, including epistatic effects, while Evolution Gym highlights the importance of expressive design representations and the difficulty of co-optimising body structure and control as the search space grows \citep{miras2026epistatic,bhatia2021evolution}. In this project, grammar masking is introduced as a method contribution: it enforces topological and kinematic constraints, such as valid port adjacency, before a candidate is sent to physics simulation. This implementation choice is intended to reduce evaluations of plainly illegal designs; it is not presented as an established conclusion of the cited literature.
...
Real-world robot design inherently involves balancing conflicting performance requirements. A morphology optimised purely for forward locomotion speed often exhibits poor stability or excessive structural complexity. Multi-Objective Evolutionary Algorithms (MOEAs) address this by discovering a Pareto front, a set of non-dominated solutions representing optimal trade-offs between competing criteria.

Standard multi-objective algorithms, such as NSGA-II, rely on crowding distance metrics to maintain population diversity along the Pareto front. However, when the optimisation problem involves more than three objectives (many-objective optimisation), crowding distance metrics lose selectivity because almost all individuals become non-dominated relative to one another. To resolve this, Deb and Jain (2014) developed NSGA-III, which replaces crowding-distance sorting with reference-direction-based niching \citep{deb2014evolutionary}. By projecting candidate solutions onto uniformly distributed reference lines in objective space, NSGA-III maintains high population diversity and convergence pressure across higher-dimensional trade-off surfaces.

In robotic co-design, integrating machine learning with MOEAs is relevant when the design space is constrained. Ming et al. (2024) provides a related example of deep reinforcement learning selecting genetic operators in constrained multi-objective optimisation \citep{ming2024constrained}. This project applies NSGA-III to a four-objective optimisation landscape:
\begin{enumerate}
    ...
    \item Maximize/minimise light-based pheromone yaw response ($f_3$),
    \item Maximize/minimise light-based pheromone speed response ($f_4$).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Unified Structural Edit Policy:} Rather than using graph policies strictly to produce motor control torques, the PPO agent uses the Graph Transformer encoder to directly output probability distributions over discrete structural edits (mutation and crossover subtree operations) across arbitrarily sized modular genotypes.
\end{enumerate}
...
Within the supplied corpus, no directly comparable study is identified that combines (a) policy-guided evolutionary graph editing for both structural mutation and crossover, (b) high-dimensional (4-objective) NSGA-III Pareto search, and (c) domain-specific kinematic, folding, and light-response grammar constraints within a unified, physics-in-the-loop co-design pipeline. This is the gap addressed by the method in this thesis. It is a conclusion about the supplied corpus, not an exhaustive claim about all prior work.

The next chapter describes the methodology used to address this gap. The later chapters evaluate whether the framework can discover functional Roblet morphologies that would be difficult to design by hand.
...
Locomotion rollout duration & $7\,\mathrm{s}$ per gait rollout; each individual runs 4 such rollouts (3-candidate $B$-sweep plus 1 winner re-run), plus 3 separate light-response stages ($7\,\mathrm{s}$ baseline, $10\,\mathrm{s}$ left-stimulus, $10\,\mathrm{s}$ front-stimulus). The total simulated time per individual is close to $55\,\mathrm{s}$. Settling ends after $\le 300$ steps or once max joint speed is $<10^{-3}\,\mathrm{rad\,or\,m/s}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The design variables include module type, port adjacency, module count, ordinary hinge angle, light-sensitive joint selection and light-detection hinge angle. The hinge angle controls the folded geometry, while the light angle controls the response produced when a selected joint receives light. Keeping these variables in one genotype allows the search to evaluate their interaction rather than optimising shape and response separately. Table~\ref{tab:design-variables} summarises the domain of each variable, and Figure~\ref{fig:genotype} shows an example half-genotype graph encoding these variables. Figure~\ref{fig:morphologies_example} shows the corresponding 2D planar layout and its folded 3D phenotype.
...
\label{eq:objectives}
\end{equation}
Here, $v_b$ is the baseline folded-gait velocity, $\tilde H_{2D}$ and $\tilde H_{3D}$ are the normalised shape entropies before and after folding, $\Delta\psi_b$ and $\Delta\psi_L$ are the baseline and left-stimulus yaw changes, and $v_F$ is the velocity under the front stimulus. The objective vector is evaluated after folding and actuation: the first component measures folded-gait velocity, the second measures the change in normalised shape entropy between the flat and folded forms, and the final two components measure changes in yaw and speed under light stimulation relative to their baselines. All four components are maximised in the attractive run; in the repulsive run, the signs of the two light-response preferences are reversed by the objective configuration rather than by changing the measured signed quantities.
...
\begin{itemize}
    ...
    \item \textbf{Collision and instability outcomes excluded from the reward stream:} An earlier version recorded a fixed penalty whenever a proposed child collided or failed simulation, giving the policy a direct learning signal against those outcomes. This was removed rather than rescaled. A reward mechanism that compares outcomes across action types can exploit the fact that every structural action (Add Node, Graft Subtree, and so on) carries some baseline collision risk, while several non-structural actions (e.g.\ Toggle Light Sensor) cannot trigger the collision gate. In practice, the policy collapsed onto the action types that could not be penalised, regardless of the penalty scale (Fig.~\ref{fig:rl-cp-action-distribution}). Collision and instability outcomes are now excluded from the reward stream entirely. Neither the collision gate's rejections nor a post-simulation instability flag is recorded as a reward. The policy therefore receives a signal based on scalarised design-quality change rather than on those failure outcomes. Both arms still pay the same wasted-retry cost when a proposed child fails the collision gate; only the RL arm's reward signal is affected.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{rl_cp_action_distribution.png}
    \caption{Breeding-decision action distribution per generation for the earlier flat-penalty reward design. By generation 150--200, almost every decision is Mutate Hinge Angle or Mutate Fold Type. This collapse motivated removing collision outcomes from the reward stream entirely.}
...
Excluding collision and instability outcomes from the reward stream prevents the reward for one action type from being measured against a shared baseline that a handful of risky outcomes could dominate. This does not guarantee equally reliable estimates for every action type. Actions sampled less often (Section~\ref{sec:rl-diagnostics} reports the exact counts) still have noisier reward and advantage estimates because they have fewer observations. That remains a limitation discussed in Section~\ref{sec:rl-diagnostics}.
...
\begin{figure}[H]
    ...
    \label{fig:pipeline}
\end{figure}
Figure~\ref{fig:pipeline} summarises this end-to-end loop. The implementation evaluates individuals in parallel operating-system processes. For each generation, graph data are converted to MJCF files, checked and passed to MuJoCo workers. Results are stored with the individual and are used by both the PPO update and NSGA-III selection. Checkpoints contain the generation, population, random-number state, training state and objective-normalisation information, allowing a run to resume without restarting from the initial population.

An evaluation cache hashes deterministic graph evaluations and avoids simulating unchanged individuals again, including carried-over survivors. The project summary reports approximately 2 minutes per generation with parallel evaluation on a machine with 6 physical cores.

\subsection{Simulation and Training Configuration}
The main reported runs (Chapter~\ref{ch:results}) used a population of 30 individuals over 300 generations, matching the generation axis of Figs.~\ref{fig:convergence}--\ref{fig:pareto-front}. Each gait rollout lasted seven seconds, but an individual was evaluated more than once. The magnetic-field sweep used three candidate field strengths (0.001, 0.008 and 0.025\,T), followed by a final run at the winning field. The three light-response stages (Section~\ref{sec:pheromone-tests}) then ran for seven seconds (baseline), ten seconds (left stimulus) and ten seconds (front stimulus). The total simulated time per individual was therefore well above seven seconds. The fresh-run random seed was 42. Every result reported in this thesis, including the RL-versus-standard-breeding comparison (Section~\ref{sec:rl-vs-random}), comes from a single seed per configuration, so variation between independent runs under the same settings has not been measured. This is discussed as a limitation in Section~\ref{sec:limitations}.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{GA convergence: best and mean scalarised population fitness across generations, attractive versus repulsive configuration.}
...
\end{figure}
The hypervolume curve in Fig.~\ref{fig:hypervolume} rises in a step-like, largely monotonic pattern across the full 300-generation run, from close to zero to approximately 0.042. The largest single steps occur around generations 90--95, 190--195 and 240--250, and the curve is still rising in the final generations recorded. It had not visibly plateaued by the end of the run. The four per-objective best-individual curves in Fig.~\ref{fig:fitness-trends} converge on different schedules: folded-gait velocity and yaw response reach a stable plateau by around generation 90--130, whereas entropy gain and speed response continue increasing in steps through most of the run and are still rising at its end. The scalarised best-fitness curve in Fig.~\ref{fig:convergence} is less monotonic than hypervolume. It peaks around generation 55, falls back over the following generations, and settles into a lower band for most of the run. This is expected because scalarisation compresses four objectives with different convergence schedules into one number. The scalarised value can fall while hypervolume continues to rise because hypervolume tracks the whole non-dominated set. The repulsive configuration (Figs.~\ref{fig:convergence-repulsive} and~\ref{fig:hypervolume-repulsive}) follows a broadly similar step-like hypervolume pattern, with comparably placed generation-90--130 and generation-190--250 steps, but its hypervolume curve flattens into a narrow band for roughly the final third of the run rather than continuing to rise, and its scalarised best-fitness curve recovers after its early peak to end close to that peak rather than settling into a markedly lower band. This suggests the repulsive configuration approaches a stable non-dominated set earlier than the attractive configuration does, although the two curves are not on a common objective scale and so are not directly comparable in magnitude. The results support the implementation of the evolutionary loop and provide evidence for O-4, although they do not show that the population has reached long-term convergence.
...
\end{figure}
The recorded non-dominated set contains solutions with different compromises between speed, folding complexity and light response. A four-objective problem can produce many non-dominated individuals because one candidate may be better in one objective and worse in another. This dominance resistance is expected for a modest population and should not be interpreted as proof that all designs are equally useful. In the $f_1$ vs $f_2$ panel of Fig.~\ref{fig:pareto-front}, most non-dominated individuals cluster at high folded-gait velocity with either high or near-zero entropy gain, rather than at intermediate values of both. This suggests a trade-off between fast locomotion and folding complexity. The designs tend to favour one rather than combining both to a moderate degree. The $f_3$ vs $f_4$ panel shows the great majority of points already saturated near the maximum recorded speed response regardless of yaw response, so speed response is rarely the binding constraint on this front. Point colour (generation) is spread across the full run in every panel, indicating that the non-dominated set is populated by individuals recorded throughout the run rather than only in its final generations.

The repulsive front (Fig.~\ref{fig:pareto-front-repulsive}) shows a less saturated $f_1$ vs $f_2$ relationship, with non-dominated individuals spread more continuously across folded-gait velocity rather than clustering at its extremes. Its $f_3$ vs $f_4$ panel is the reverse of the attractive front's: most points sit near the minimised end of the speed-response axis rather than saturating near a maximum, consistent with the repulsive configuration optimising the two light-response objectives in the opposite direction. As in the attractive front, point colour is spread across the full run, so this front is also not the product of only the final generations.
...
\end{figure}
The entropy--velocity plot addresses AO-2 by comparing a measure of change during folding with the resulting gait speed. In Fig.~\ref{fig:entropy-velocity}, the individuals with the highest entropy gain ($\Delta H \gtrsim 0.18$) are concentrated at near-zero velocity. Two late-generation exceptions combine a comparable entropy gain ($\Delta H \approx 0.19$--$0.20$) with a moderate velocity of around 0.5\,m/s. Conversely, the two highest-velocity individuals recorded, at around 1.25 and 1.55\,m/s, both have a low entropy gain close to 0.02. Most of the population sits at low-to-moderate velocity (0--0.6\,m/s) with a low entropy gain (0--0.03). The pattern suggests that high folding-complexity gain is difficult to combine with fast locomotion, although the two late-generation examples show that the trade-off is not absolute. Since they are the only points in that region, they describe specific designs rather than a general rule established by the run. A broad low-entropy cloud spanning most of the observed velocity range would suggest that connectivity and actuation choices dominate velocity regardless of folding complexity, and the low-entropy majority is consistent with that possibility. The available evidence supports examining the relationship, but it does not justify a causal claim. No statistical test of association has been applied to these data.
...
Figure~\ref{fig:morphology-grid} tracks four individual lineages from generation 0 to generation 299 in each configuration. All four repulsive lineages become visibly elongated by generation 150 and remain so at generation 299, forming curved, single-chain ``worm'' or ``thread-like'' morphologies instead of the more compact, radially branched starting shapes. The attractive run shows less consistent elongation. Two of the four lineages flatten into a similar wavy chain by generation 299, while the other two remain closer to a compact, cross-like arrangement of short branches around the root module. Within this small sample, elongation appears more strongly favoured by the repulsive objectives.
...
The morphology figures provide evidence for O-1 to O-3. Graph designs were generated, converted into assemblies, folded and evaluated in the physics simulator. They also show how AO-1 changed from surrogate fitness prediction to light-response objectives. Both rendered individuals in Fig.~\ref{fig:evolved-morphologies-both} are bilaterally symmetric, as expected from the mirror construction (Section~\ref{sec:symmetry}). Light-sensitive modules are green and non-light-sensitive modules are blue, matching the 8-of-13 and 19-of-38 counts reported in Table~\ref{tab:evolved-morphology-values}. The attractive individual is a compact, arch-shaped assembly. The repulsive individual is a long, curved chain with roughly three times as many modules, matching the elongation pattern in Fig.~\ref{fig:morphology-grid} for this pair of selected individuals. The two response modes should therefore be read as separate design problems, not as proof that one morphology can optimise both behaviours at once.
...
\begin{figure}[H]
    ...
    \caption{Best and mean scalarised fitness per objective, RL-assisted versus standard breeding, over a 300-generation comparison run.}
    \label{fig:fitness-trends-comparison}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{convergence_comparison.png}
    \caption{GA convergence: best and mean scalarised population fitness across generations, RL-assisted versus standard breeding.}
...
The four metrics shown do not agree on a single winner at this scale, so they are reported as measured rather than reduced to one verdict. On per-objective best fitness (Fig.~\ref{fig:fitness-trends-comparison}), the RL-assisted arm reaches a clearly higher best folded-gait velocity (approximately 1.57 vs.\ 0.57\,m/s by the run's end) and a higher best speed response (approximately 280 vs.\ 190) than the standard arm. The two arms reach a similar best yaw response, both saturating near the $179$-$180^\circ$ ceiling by around generation 90. Best entropy gain is broadly comparable between the arms for most of the run, with the standard arm briefly ahead in the final generations. On scalarised best fitness (Fig.~\ref{fig:convergence-comparison}), the RL-assisted arm leads clearly for roughly the first 90 generations, the two arms interleave closely from about generation 90 to 250, and the standard arm is modestly ahead for the remainder of the run. Hypervolume is the primary many-objective convergence measure adopted in Section~\ref{sec:evo-learning}. In Fig.~\ref{fig:hypervolume-comparison}, the standard arm is higher than the RL-assisted arm for almost the entire run, ending at approximately 0.065--0.08 versus approximately 0.04. This indicates a larger or better-spread non-dominated front at the population level despite the RL arm's stronger individual best values on some objectives. On collision rate (Fig.~\ref{fig:collision-rate-comparison}), the two arms are similar and noisy (roughly 0.2--0.5) for about the first 130 generations. After that, the RL-assisted arm's collision rate drops and stays consistently lower, mostly below 0.2, while the standard arm continues to fluctuate across its earlier range.

These results are not contradictory once it is recalled what each metric measures: an RL-guided policy that concentrates variation on the specific edits that push a few objectives to strong best-individual values, while increasingly favouring lower-risk parameter mutations in later generations (Section~\ref{sec:rl-diagnostics}), can produce strong best-of-run values on some objectives without necessarily producing a larger or more diverse non-dominated population front, which is what hypervolume tracks. Read this way, the RL-assisted policy has not demonstrated an unqualified advantage over standard breeding at this scale: it wins clearly on two of four individual objectives and on collision avoidance in the second half of the run, but loses on this thesis's own primary convergence measure (hypervolume) and on scalarised best fitness in the run's final third. This is a more nuanced result than either ``RL is better'' or ``RL is worse,'' and a stronger conclusion in either direction would require multiple independent seeds under matched generation budgets, which is proposed as future work (Chapter~\ref{ch:conclusion}).

The action-distribution plots (Fig.~\ref{fig:action-distribution-both}) show that all ten grammar actions remain in use throughout the 300 generations after collision outcomes were excluded from the RL reward stream (Section~\ref{sec:rl-actor-critic}). The earlier collapse onto one or two actions does not reappear (Fig.~\ref{fig:rl-cp-action-distribution}). The two crossover actions, Graft Subtree and Swap Subtrees, make up a smaller but persistent share of decisions than the mutation actions in both arms. Section~\ref{sec:rl-diagnostics} quantifies this imbalance from the action sample counts.
...
Because the collision penalty has been removed from the reward stream, the per-action-type reward-normalisation idea proposed for the old collision-driven collapse no longer applies in that form. Crossover actions receive roughly a fifth to a sixth of the samples assigned to the most common mutations (Fig.~\ref{fig:rl-diagnostics-by-action}). Their reward and advantage estimates therefore rely on less evidence. Chapter~\ref{ch:conclusion} proposes a broader remedy: give the less frequently sampled actions a more reliable reward signal.
...
O-1 was achieved with a qualified result. The graph policy and its integration were implemented, and the 300-generation comparison in Section~\ref{sec:rl-vs-random} shows a mixed outcome. The RL-assisted arm reaches a higher best individual velocity and speed response than standard breeding, and its collision rate is substantially lower from around generation 140 onward. Standard breeding nevertheless maintains a consistently higher hypervolume and a modestly higher scalarised best fitness in the final third of the run (Figs.~\ref{fig:fitness-trends-comparison}--\ref{fig:collision-rate-comparison}). Once collision outcomes were excluded from the reward stream, the earlier action-collapse failure mode did not recur over the full 300 generations, although crossover actions remain a minority of decisions with less reward evidence behind them. The ML approach was therefore implemented and evaluated at full scale, but it did not uniformly outperform the random baseline. A same-generation-budget, multi-seed comparison is still needed. O-2 and O-3 were achieved at simulation level: legal graphs were folded, actuated and scored in MuJoCo, as illustrated by the evolved assemblies in Fig.~\ref{fig:evolved-morphologies-both}. O-4 was addressed through the convergence, hypervolume and Pareto analyses (Figs.~\ref{fig:convergence}, \ref{fig:hypervolume} and \ref{fig:pareto-front}). Hypervolume was still rising at the end of the 300-generation run, so the evidence supports the framework's operation without establishing long-term convergence.

\section{Advanced Objectives (AO-1 and AO-2)}
\label{sec:disc-advanced}
AO-1 was changed and achieved through the attractive and repulsive pheromone-response objectives and their morphology results (Fig.~\ref{fig:evolved-morphologies-both}), including the elongation difference between the two configurations (Fig.~\ref{fig:morphology-grid}). AO-2 was addressed by comparing entropy gain with folded-gait velocity (Fig.~\ref{fig:entropy-velocity}). High entropy gain is concentrated at near-zero velocity, with a small number of later-generation exceptions. This suggests a trade-off, but it is not statistically established without longer, multi-seed runs. The learned variation policy performs better on some objectives and on collision avoidance, but not on hypervolume. The uneven amount of reward evidence available for each action type (Section~\ref{sec:rl-diagnostics}) may contribute to this mixed result.

\section{Findings}
\label{sec:findings}

\subsection{Morphology Diversity Across Primitive Behaviours}
The results of Chapter~\ref{ch:results} show that the framework is intended to find more than a single high-scoring locomotion design. It should also find morphologies that express folded-gait locomotion and pheromone-driven yaw or speed response. These behaviours are difficult to design by hand because their relationship to graph structure is not obvious. The selected individuals in Section~\ref{sec:evolved-morphologies} (Table~\ref{tab:evolved-morphology-values}) show that the framework can find structurally different morphologies with strong performance in different primitives. The attractive individual reaches a near-ceiling yaw response of $179.3^{\circ}$, while the repulsive individual has a folding-complexity gain of $\Delta H = 0.2307$. As noted in Section~\ref{sec:evolved-morphologies}, neither individual optimises all primitives at once.
...
The elongated, thread-like morphologies observed under the repulsive configuration (Fig.~\ref{fig:morphology-grid}) appear to be favoured partly because a longer body can carry more light-sensitive joints. This increases the light-response objectives without a corresponding constraint on practicality. Such morphologies are unlikely to be realisable as working physical Roblet units. Possible reasons include self-assembly time, structural rigidity and the physical routing of the magnetic bonding sites. The project's objectives did not capture this gap between simulated fitness and physical buildability. These elongated structures also move less freely than the more compact morphologies shown in Fig.~\ref{fig:old-evolved-morphology-both}, tending to twist and turn during locomotion rather than travelling in a controlled, straight path.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=0.9\linewidth]{phermone_attracting_evolved_morph.png}
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=0.9\linewidth]{Phermone_repelling_evolved_morph.png}
        \caption{Evolved morphology in the repulsive configuration.}
        \label{fig:old-evolved-morphology-repulsive}
    \end{subfigure}
    \caption{Compact evolved morphologies from an early generation, attractive and repulsive configurations (compare with the elongated morphologies in Fig.~\ref{fig:morphology-grid}).}
...
\label{app:algo}
The PPO-guided actor-critic (Section~\ref{sec:rl-actor-critic}) and the NSGA-III environmental selection (Section~\ref{sec:evo-learning}) are the two algorithmic components original to this project; their exact configuration is tabulated here in full, since this level of detail would interrupt the narrative in Chapter~\ref{ch:methodology} but is needed to reproduce a run exactly. Table~\ref{tab:rl-hyperparams-appendix} lists the PPO encoder and optimisation hyperparameters, Table~\ref{tab:state-space-appendix} the per-node state representation passed into that encoder, and Table~\ref{tab:nsga3-appendix} the NSGA-III selection configuration.
...
\label{app:params}
Table~\ref{tab:parts-appendix} gives the mass and bounding-box dimensions of each of the six moulded part types, as exported from the Fusion 360 CAD assembly. These values are converted into the per-part masses used by the MuJoCo digital twin (Section~\ref{sec:digital-twin}). Only mass and inertia are read from the CAD export during simulation setup. The body-local centre-of-mass offset used to position each part in the MJCF is fitted separately from the CAD assembly, so it is not a value read directly from the export file. A future reader extending the pipeline to a new part should account for this distinction.
